---
title: "Spring Boot"
description: "Spring Boot 的历史学习笔记。"
date: 2026-05-21
tags: ["Spring Boot", "框架"]
keywords: ["Spring Boot", "框架", "Spring Boot"]
legacy_source: "框架/Spring/SpringBoot/尚硅谷SpringBoot/Spring Boot.md"
---
# **一、**Spring Boot 入门
...
```shell
[root@localhost ~]# docker run --name mysql01 -e MYSQL_ROOT_PASSWORD=[REDACTED] -d mysql
...
```shell
[root@localhost ~]# docker run -p 3306:3306 --name mysql02 -e MYSQL_ROOT_PASSWORD=[REDACTED] -d mysql
...
```
docker run --name mysql03 -v /conf/mysql:/etc/mysql/conf.d -e MYSQL_ROOT_PASSWORD=[REDACTED] -d mysql:tag
把主机的/conf/mysql文件夹挂载到 mysqldocker容器的/etc/mysql/conf.d文件夹里面
改mysql的配置文件就只需要把mysql配置文件放在自定义的文件夹下（/conf/mysql）

docker run --name some-mysql -e MYSQL_ROOT_PASSWORD=[REDACTED] -d mysql:tag --character-set-server=utf8mb4 --collation-server=utf8mb4_unicode_ci
指定mysql的一些配置参数
```
